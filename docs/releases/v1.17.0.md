...
- [[#135](https://github.com/sighupio/module-policy/pull/135)] Adds `name: http` to the `gatekeeper-policy-manager` Service port definition so that the Ingress backend port reference by name resolves correctly when using HAProxy Kubernetes Ingress Controller.
